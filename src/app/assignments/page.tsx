...
export default function AssignmentsPage({ searchParams }: { searchParams?: { page?: string } }) {
  ...
  useEffect(() => {
    if(firestore) {
        const countQuery = query(collection(firestore, 'assignments'));
        ...
        if (currentPage > 1) {
            const fetchLastVisible = async () => {
                const firstPageQuery = query(collection(firestore, 'assignments'), orderBy('date', 'desc'), limit(PAGE_SIZE * (currentPage - 1)));
                ...
                {assignments.map((assignment: Assignment) => (
                    ...
                    <CardHeader>
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                            <div>
                                ...
                                <CardDescription>
                                    Posted {format(assignment.date.toDate(), 'MMMM d, yyyy')}
                                </CardDescription>
                            </div>
                    ...
                    <CardContent>
                        ...
                        <p className="font-semibold text-sm">
                            ...
                            <span className="text-muted-foreground"> ({formatDistanceToNow(assignment.deadline.toDate(), { addSuffix: true })})</span>
                        </p>
                        {assignment.submitted && (
                            <div className="mt-2 flex items-center text-green-600">
                                <CheckCircle className="h-4 w-4 mr-2" />
                                <span className="text-sm font-medium">Submitted on {format(assignment.submissionDate.toDate(), 'PP')}</span>
                            </div>
                        )}
                    </CardContent>
